...
class FeatureVector:
    ...
    def to_array(self) -> list[float]:
        ...
        return [
            self.f_volume_delta,
            self.f_polarity_added, self.f_polarity_removed,
            float(self.f_charge_delta),
            self.f_aromatic_added,
            self.f_hbond_donor_gained, self.f_hbond_acceptor_gained,
            float(self.f_contact_count_delta),
            float(self.f_atoms_lost), float(self.f_atoms_gained),
            self.f_nearest_ligand_distance,
            float(self.f_neighborhood_4a_count),
            self.f_is_catalytic, self.f_is_ligand_contact, self.f_is_pocket_lining,
            float(self.f_pocket_catalytic_in_4a),
            float(self.f_ligand_heavy_atoms),
            float(self.f_ligand_hbond_donors), float(self.f_ligand_hbond_acceptors),
...
def extract_features(
    ...
    return FeatureVector(
        ...
        f_direct_evidence_count=sum(
            1 for entries in lit.get("by_grade", {}).values()
            for e in entries
