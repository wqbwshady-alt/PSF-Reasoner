...
def _protein_features(matrix):
    ...
    samples, labels = [], []
    for s, label in zip(matrix.samples, matrix.labels, strict=True):
        ...
        for i, fam in enumerate(["P03367", "P00374", "P62593", "P00533", "P00519"]):
            ...
            if i == 0:
                fv.f_polarity_added = val
            elif i == 1:
                fv.f_aromatic_added = val
            elif i == 2:
                fv.f_hbond_donor_gained = val
            elif i == 3:
                fv.f_hbond_acceptor_gained = val
            elif i == 4:
                fv.f_is_catalytic = val
        samples.append(fv)
        labels.append(label)
...
def _ligand_features(matrix):
    ...
    for s, label in zip(matrix.samples, matrix.labels, strict=True):
        ...
        fv.f_ligand_heavy_atoms = (
            1 if lid in ("MK1", "DRV", "APV", "NFV", "SQV", "IDV") else 0
        )  # HIV inhibitors
        samples.append(fv)
        labels.append(label)
    return samples, labels
        ...
        for i, name in enumerate(all_names):
            if i in keep_indices and i < len(arr):
                ...
                if name == "volume_delta":
                    fv.f_volume_delta = arr[i]
                elif name == "contact_count_delta":
                    fv.f_contact_count_delta = int(arr[i])
                elif name == "atoms_lost":
                    fv.f_atoms_lost = int(arr[i])
                elif name == "atoms_gained":
                    fv.f_atoms_gained = int(arr[i])
                elif name == "nearest_ligand_distance":
                    fv.f_nearest_ligand_distance = arr[i]
                elif name == "neighborhood_4a_count":
                    fv.f_neighborhood_4a_count = int(arr[i])
                elif name == "is_catalytic":
                    fv.f_is_catalytic = int(arr[i])
                elif name == "is_ligand_contact":
                    fv.f_is_ligand_contact = int(arr[i])
                elif name == "is_pocket_lining":
                    fv.f_is_pocket_lining = int(arr[i])
                elif name == "pocket_catalytic_in_4a":
                    fv.f_pocket_catalytic_in_4a = int(arr[i])
        samples.append(fv)
        labels.append(label)
    return samples, labels
    ...
    for s, label in zip(matrix.samples, matrix.labels, strict=True):
        ...
        for i, name in enumerate(all_names):
            if name in struct_names and i < len(arr):
                if name == "volume_delta":
                    fv.f_volume_delta = arr[i]
                elif name == "polarity_added":
                    fv.f_polarity_added = int(arr[i])
                elif name == "polarity_removed":
                    fv.f_polarity_removed = int(arr[i])
                elif name == "charge_delta":
                    fv.f_charge_delta = int(arr[i])
                elif name == "aromatic_added":
                    fv.f_aromatic_added = int(arr[i])
                elif name == "hbond_donor_gained":
                    fv.f_hbond_donor_gained = int(arr[i])
                elif name == "hbond_acceptor_gained":
                    fv.f_hbond_acceptor_gained = int(arr[i])
                elif name == "contact_count_delta":
                    fv.f_contact_count_delta = int(arr[i])
                elif name == "atoms_lost":
                    fv.f_atoms_lost = int(arr[i])
