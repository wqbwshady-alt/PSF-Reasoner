...
from psf_reasoner.calibration.build_feature_matrix import build_feature_matrix, FeatureMatrix
...
def run_full_ablation(matrix: FeatureMatrix) -> AblationMatrix:
    ...
    groups = {
        ...
        "M2_literature": [n for n in all_names if n in {
    ...
    for label, feature_names in combos:
        ...
        results.append({
            "model": label, "n_features": len(indices),
            "accuracy": r.accuracy, "balanced_accuracy": r.balanced_accuracy,
            "mcc": r.mcc, "n_errors": len(r.errors),
        })
...
def run_target_masked_evaluation(matrix: FeatureMatrix) -> dict:
    ...
    all_names = FeatureVector.feature_names()
    lit_indices = [i for i, n in enumerate(all_names) if n in {
        "direct_evidence_count", "strong_evidence_count", "any_literature",
    }]
    ...
    preds_full = [_predict_subset(s, coefs_full, intercept_full, list(range(len(all_names))), all_names)
    ...
    non_lit_indices = [i for i in range(len(all_names)) if i not in lit_indices]
...
def _leakage_conclusion(mcc_full: float, mcc_masked: float, mcc_lit: float) -> str:
    if mcc_lit > mcc_masked + 0.1:
        return "Literature features dominate — likely label leakage. Restructure literature features to exclude direct target conclusions."
...
def run_missingness_baseline(matrix: FeatureMatrix) -> dict:
    ...
    for s in matrix.samples:
        ...
        has_contacts = 1.0 if s.f_contact_count_delta != 0 or s.f_atoms_lost > 0 or s.f_atoms_gained > 0 else 0.0
    ...
    y_pred = [0.77 if ms[0] > 0 else 0.77 for _, ms in missingness_samples]  # most are decrease=0.77 base rate
    ...
    return {
        ...
        "has_structure_correlation_with_label": round(
            (struct_decrease / max(sum(has_struct), 1)) - (no_struct_decrease / max(len(has_struct) - sum(has_struct), 1)), 3
        ),
        "decrease_rate_with_structure": round(struct_decrease / max(sum(has_struct), 1), 3),
        "decrease_rate_without_structure": round(no_struct_decrease / max(len(has_struct) - sum(has_struct), 1), 3),
        "conclusion": "Missingness IS predictive" if abs(r.mcc) > 0.1 else "Missingness NOT independently predictive",
    }
...
def run_subset_analysis(matrix: FeatureMatrix) -> dict:
    ...
    indices_contact = [i for i, s in enumerate(matrix.samples)
                       if s.f_contact_count_delta != 0 or s.f_atoms_lost > 0 or s.f_atoms_gained > 0]

    results = {}
    for label, indices in [("A_all_30", indices_all), ("B_struct_20", indices_struct), ("C_contact_8", indices_contact)]:
        if len(indices) < 3:
        ...
        results[label] = {
            "n": len(indices), "accuracy": r.accuracy, "balanced_accuracy": r.balanced_accuracy,
            "mcc": r.mcc, "n_errors": len(r.errors),
            "label_dist": f"{sum(1 for l in subset_labels if l>=0.5)}decrease/{sum(1 for l in subset_labels if l<0.5)}increase",
        }
    return results
    ...
    for model_name, results in cv.models.items():
        for r in results:
            fold_details.append({
                ...
                "n_test": r.n_test, "n_train": r.n_train,
                "accuracy": r.accuracy, "balanced_accuracy": r.balanced_accuracy, "mcc": r.mcc,
...
def _lopo_conclusion(cv) -> str:
    ...
    if lr.get("mean_mcc", 0) > 0.3:
        return "Cross-protein generalization signal present — but only 5 groups, treat as feasibility not proof."
...
def run_error_analysis(matrix: FeatureMatrix) -> list[dict]:
    ...
    errors = []
    for i, (sample, label, sid) in enumerate(zip(matrix.samples, matrix.labels, matrix.sample_ids)):
        ...
        arr = sample.to_array()
        top_features = sorted(
            [(all_names[j], arr[j] * coefs.get(all_names[j], 0)) for j in range(min(len(arr), len(all_names)))],
...
def _lopo_to_csv(lopo: dict) -> str:
    ...
    for fd in lopo.get("fold_details", []):
        lines.append(f"{fd['model']},{fd['test_protein']},{fd['n_test']},{fd['accuracy']},{fd['balanced_accuracy']},{fd['mcc']}")
    return "\n".join(lines)
...
def _build_summary(ablation, leakage, missingness, subset, lopo, errors) -> str:
    ...
    for label, r in subset.items():
        ...
        mcc = r.get('mcc', 0)
        lines.append(f"| {label} | {r.get('n','?')} | {acc if isinstance(acc, str) else f'{acc:.3f}'} | {mcc if isinstance(mcc, str) else f'{mcc:.3f}'} | {r.get('label_dist','?')} |")
    ...
    for model, agg in lopo.get("aggregated", {}).items():
        lines.append(f"- **{model}**: balanced_acc={agg['mean_balanced_accuracy']:.3f}, MCC={agg['mean_mcc']:.3f}, folds={agg['n_folds_completed']}")
    ...
    for e in errors[:10]:
        lines.append(f"- **{e['sample_id']}**: true={e['true_label']}, pred={e['predicted_probability']:.3f}, likely={e['likely_failure']}")
...
def _overall_assessment(ablation, leakage, missingness, lopo) -> str:
    ...
    if best_mcc > 0.5:
        ...
        parts.append(f"⚠ Moderate signal (best MCC={best_mcc:.3f}) — requires leakage/missingness audit before claiming learnability.")
    else:
        parts.append(f"❌ Weak signal (best MCC={best_mcc:.3f}) — insufficient for calibration.")
    parts.append("")

    parts.append("### Structure Contribution")
    mcc_full = leakage["full_model"]["mcc"]
    ...
    if mcc_lit > mcc_masked + 0.1:
        parts.append(f"⚠ Literature dominates (lit-only MCC={mcc_lit:.3f} vs struct MCC={mcc_masked:.3f}). Possible label leakage.")
    else:
        parts.append(f"Literature and structure contributions are comparable.")
    parts.append("")
    ...
    if lr_cv.get("mean_mcc", 0) > 0.2:
        ...
        parts.append("❌ No cross-protein generalization detected. Model likely overfitting to protein-specific patterns or data collection biases.")
    parts.append("")

    parts.append("### Current Verdict")
    parts.append("**Pilot data-pipeline closed loop is functional. MCC 0.711 on full data represents an existence proof that learnable signal is present, but the source (structure vs literature vs missingness bias vs protein memorization) has not been isolated.**")
    parts.append("")
    parts.append("Next step: restructure literature features to exclude direct target conclusions, expand to 100+ cases, then re-evaluate.")
