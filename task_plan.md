...
| 18. Validated interaction engine | partial | Typed/heuristic interaction classifiers exist for hydrogen bonds (with H-position estimation + heavy-atom angle proxy fallback), salt bridges, pi interactions, hydrophobic contacts, and water bridges. Donor/acceptor tables, atom-typing logic, and geometry rules are in place. NOT YET externally validated against PLIP, Arpeggio, HBPLUS, or DSSP. Aromaticity is name/connectivity-based. Water bridges use distance-only partner selection. H-bond angle uses estimated (not observed) H positions with 90° proxy fallback (vs standard 110°). |
...
| 20. Automatic mutation modelling and local relaxation | partial | A conservative local side-chain modeler handles truncation-only cases (target residue is a subset of source, e.g., V→A, F→A). It removes incompatible atoms but cannot add atoms for gain-of-size mutations (A→V, A→F). No rotamer sampling or backbone relaxation. Modelled evidence is marked as modelled computation, distinct from experimental structures. General mutation modelling and local relaxation are not yet implemented. |
...
| 22. Evidence-aware PSF reasoning | partial | Mechanism ranking and confidence respond to computed physical evidence (distance deltas, interaction counts, SASA, pocket, network, energy scores). This is evidence-responsive heuristic ranking — NOT calibrated evidence-aware inference. Confidence values are explicitly marked as uncalibrated heuristic scores. Contradictory evidence and alternatives are surfaced but do not drive quantitative probability updates. |
| 23. Experimental validation benchmark | partial | One qualitative V82A/MK1 calibration label exists, sourced from RCSB 1SDT/1SDV and Mahalingam et al. 2004. No numeric Ki/Kd/IC50 values are invented. No multi-case dataset, no held-out test set, no mechanism-ranking accuracy measurement, no failure-mode analysis. This is a qualitative calibration fixture — NOT an experimental validation benchmark. |
...
| Git status is unavailable because the workspace is not a Git repository | 1 | [HISTORICAL — 2026-07-10] At the start of the project the workspace had no Git metadata. Repository was initialized on 2026-07-13 and as of 2026-07-14 has 17 commits with clean working tree on `main`. |
