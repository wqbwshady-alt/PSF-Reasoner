...
def test_v82a_bidirectional_chain_is_complete_and_consistent(
    ...
    # REQUIRED evidence is generated by the reverse reasoner — exact types
    # depend on whether baseline or LLM reasoner is active.
    assert any(item.status is EvidenceStatus.REQUIRED for item in report.physical_evidence), (
        "report should contain at least one REQUIRED evidence item"
    )
    assert any(
